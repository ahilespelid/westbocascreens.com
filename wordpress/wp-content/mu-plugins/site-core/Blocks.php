...
final class Blocks
{
     ...
     *
     * Автозапуск решает вызывающий код через $video['autoplay']: включённый значит,
     * что ролик встроен в общую для всей страницы систему video.js — играет со звуком,
     * когда попадает в зону видимости, и ставится на паузу, как только в кадр попадает
     * другой такой же ролик (играет всегда только один, звуковой каши не бывает).
     * Выключенный autoplay — обычный нативный плеер с кнопкой play, который посетитель
     * запускает сам и video.js не трогает вовсе.
     *
